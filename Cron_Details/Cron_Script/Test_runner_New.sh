#!/bin/bash
# ----------------------------------------------------------
# 30-minute pytest run with Allure report generation
# ----------------------------------------------------------

# === CONFIG ===
PROJECT_DIR="/Kunal_Pipaliya/Project_1_CLMM_QA_Testnet"

# Directory for cron logs
LOG_DIR="$PROJECT_DIR/Cron_Details/Cron_log"
NOW=$(date '+%Y-%m-%d_%H-%M-%S')            # current datetime for log name
LOGFILE="$LOG_DIR/cron_30min_${NOW}.log"    # datetime-stamped log file

LOCKFILE="/tmp/test_runner_30min.lock"      # unique lockfile for this job
PYTHON="/usr/bin/python3"
PYTEST="/usr/local/bin/pytest"
ALLURE="/usr/bin/allure"                    # adjust if Allure binary is elsewhere

ALLURE_RESULTS="$PROJECT_DIR/Reports/Allure_Report/Allure_30min/allure-results"
ALLURE_REPORT="$PROJECT_DIR/Reports/Allure_Report/Allure_30min/allure-report"
...
# === LOCK CHECK ===
if [ -f "$LOCKFILE" ]; then
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] Another 30-min run still in progress. Exiting." \
...
# === MAIN EXECUTION ===
{
    echo "========== START 30-MIN RUN: $(date '+%Y-%m-%d %H:%M:%S') =========="
    ...
    echo "=========== END 30-MIN RUN: $(date '+%Y-%m-%d %H:%M:%S') ==========="
} >> "$LOGFILE" 2>&1
